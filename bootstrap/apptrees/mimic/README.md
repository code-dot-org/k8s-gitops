...
Nothing under `bootstrap/apptrees/mimic/` is meant to be part of the real cluster topology.
...
```sh
kubectl apply --server-side --field-manager=terraform -f <(curl -fsSL https://raw.githubusercontent.com/code-dot-org/k8s-gitops/main/bootstrap/apptrees/mimic/apps/app-of-apps/bootstrap.yaml)
```

Destroy:

```sh
kubectl delete --cascade=foreground --wait=true -f <(curl -fsSL https://raw.githubusercontent.com/code-dot-org/k8s-gitops/main/bootstrap/apptrees/mimic/apps/app-of-apps/bootstrap.yaml)
```
