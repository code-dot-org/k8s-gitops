...
It is deliberately separate from `bootstrap/apptrees/mimic/`.
`bootstrap/apptrees/mimic/` is a small Argo shape
...
```sh
kubectl apply --server-side --field-manager=terraform -f /Users/seth/src/k8s-gitops/bootstrap/apptrees/argo-trace-stress-test/roots/primary/bootstrap.yaml
```

Bootstrap the secondary root:

```sh
kubectl apply --server-side --field-manager=terraform -f /Users/seth/src/k8s-gitops/bootstrap/apptrees/argo-trace-stress-test/roots/secondary/bootstrap.yaml
```

Delete both roots foreground and wait:

```sh
kubectl delete --cascade=foreground --wait=true -f /Users/seth/src/k8s-gitops/bootstrap/apptrees/argo-trace-stress-test/roots/primary/bootstrap.yaml
kubectl delete --cascade=foreground --wait=true -f /Users/seth/src/k8s-gitops/bootstrap/apptrees/argo-trace-stress-test/roots/secondary/bootstrap.yaml
```
